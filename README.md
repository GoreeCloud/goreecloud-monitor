...
> **Current state:** source-stable deployment candidate. The source, migration tooling, hardened production Compose topology, disposable production-stack validation, and recovery gates are implemented and validated. Uptime Kuma remains the production monitoring platform until GoreeCloud Monitor completes live target-environment publication, migration/parallel acceptance, the ICMP/Ping coverage decision, target recovery testing, and an explicit cutover.
...
It is not authorization to deploy. See `docs/production-deployment.md` and `docs/icmp-reachability.md` for the target acceptance and unresolved Ping requirements.
...
See `docs/architecture.md`, `docs/deployment.md`, `docs/production-deployment.md`, `docs/uptime-kuma-migration.md`, `docs/icmp-reachability.md`, `docs/backup.md`, `docs/recovery.md`, and `SECURITY.md`.
