...
- `.env` — Compose interpolation values such as image references, protected file paths, persistent database path, gateway network name, fixed backend bridge name (`MONITOR_BACKEND_BRIDGE_NAME`), fixed backend subnet (`MONITOR_BACKEND_SUBNET`), approved private worker DNS/backend-gateway address (`MONITOR_DNS_SERVER`), and fixed worker backend address (`MONITOR_WORKER_BACKEND_IP`).
