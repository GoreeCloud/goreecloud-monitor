...
async def _certificate_expiry(host: str, port: int, timeout: float) -> datetime:
    ...
    try:
        ...
        if ssl_object is None:
            raise RuntimeError("TLS session was not established")
        cert = ssl_object.getpeercert()
        not_after = cert.get("notAfter")
        if not not_after:
            raise RuntimeError("Certificate expiration could not be determined")
...
async def _open_validated_http_response(client: httpx.AsyncClient, monitor: Monitor):
    ...
    for hop in range(6):
        ...
        if parsed.scheme not in {"http", "https"} or not parsed.hostname:
            raise ValidationError("Redirect target is not a valid HTTP or HTTPS URL")
        if parsed.username or parsed.password:
            raise ValidationError("Redirect targets containing credentials are not permitted")
        if monitor.kind == Monitor.Kind.HTTPS and parsed.scheme != "https":
            raise ValidationError("HTTPS monitors do not permit redirects that downgrade to HTTP")
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout, follow_redirects=False, verify=True, trust_env=False) as client:
            ...
            if response.status_code != monitor.expected_status_code:
                ...
                return CheckOutcome(False, Monitor.State.DOWN, elapsed, f"HTTP {response.status_code}, expected {monitor.expected_status_code}")
            body = b""
            if monitor.expected_body_text or monitor.expected_json_path:
                async for chunk in response.aiter_bytes():
                    if len(body) + len(chunk) > settings.MONITOR_MAX_RESPONSE_BYTES:
                        elapsed = (time.perf_counter() - started) * 1000
                        return CheckOutcome(False, Monitor.State.DOWN, elapsed, "Response body exceeded the configured inspection limit")
                    body += chunk
        elapsed = (time.perf_counter() - started) * 1000
        if monitor.expected_body_text:
            text = body.decode("utf-8", errors="replace")
            if monitor.expected_body_text not in text:
                return CheckOutcome(False, Monitor.State.DOWN, elapsed, "Expected response text was not found")
        if monitor.expected_json_path:
            try:
                actual = _json_path(json.loads(body.decode("utf-8")), monitor.expected_json_path)
            except (ValueError, KeyError, IndexError, TypeError, UnicodeDecodeError):
                return CheckOutcome(False, Monitor.State.DOWN, elapsed, "Expected JSON path was not available")
            if monitor.expected_json_value and str(actual) != monitor.expected_json_value:
                return CheckOutcome(False, Monitor.State.DOWN, elapsed, "Expected JSON value did not match")
        ...
        if final_parsed.scheme == "https":
            ...
            if remaining_days <= monitor.tls_warning_days:
                return CheckOutcome(True, Monitor.State.DEGRADED, elapsed, f"TLS certificate expires in {max(0, int(remaining_days))} day(s)", expires)
        ...
        elapsed = (time.perf_counter() - started) * 1000
        return CheckOutcome(False, Monitor.State.DOWN, elapsed, f"{type(exc).__name__}: {str(exc)[:420]}")
    finally:
...
async def check_tcp(monitor: Monitor) -> CheckOutcome:
    ...
    try:
        ...
        elapsed = (time.perf_counter() - started) * 1000
        return CheckOutcome(True, Monitor.State.UP, elapsed, "TCP connection succeeded")
...
async def check_dns(monitor: Monitor) -> CheckOutcome:
    ...
    try:
        answer = await resolver.resolve(monitor.target, monitor.dns_record_type.upper())
        values = sorted(str(item).rstrip(".") for item in answer)
        elapsed = (time.perf_counter() - started) * 1000
        if monitor.expected_dns_answer and monitor.expected_dns_answer.rstrip(".") not in values:
            return CheckOutcome(False, Monitor.State.DOWN, elapsed, "DNS answer did not contain the expected value")
...
async def check_push(monitor: Monitor) -> CheckOutcome:
    if not monitor.last_heartbeat_at:
        return CheckOutcome(False, Monitor.State.DOWN, None, "No heartbeat has been received")
    age = (timezone.now() - monitor.last_heartbeat_at).total_seconds()
    allowed_age = monitor.interval_seconds + monitor.heartbeat_grace_seconds
    if age > allowed_age:
        return CheckOutcome(False, Monitor.State.DOWN, None, f"Heartbeat is {int(age)}s old; limit is {allowed_age}s")
    return CheckOutcome(True, Monitor.State.UP, None, "Heartbeat is current")


async def perform_check(monitor: Monitor) -> CheckOutcome:
    if monitor.kind in {Monitor.Kind.HTTP, Monitor.Kind.HTTPS}:
        return await check_http(monitor)
    if monitor.kind == Monitor.Kind.TCP:
        return await check_tcp(monitor)
    if monitor.kind == Monitor.Kind.DNS:
        return await check_dns(monitor)
    if monitor.kind == Monitor.Kind.PUSH:
        return await check_push(monitor)
    return CheckOutcome(False, Monitor.State.DOWN, None, "Unsupported monitor type")


def _is_in_maintenance(monitor_id: int, now) -> bool:
    return MaintenanceWindow.objects.filter(monitors__id=monitor_id, starts_at__lte=now, ends_at__gte=now).exists()


def _mark_maintenance(monitor_id: int, now) -> None:
    Monitor.objects.filter(id=monitor_id).update(state=Monitor.State.MAINTENANCE, last_checked_at=now, last_message="Approved maintenance window")
...
def _apply_outcome(monitor_id: int, outcome: CheckOutcome, checked_at) -> tuple[str | None, str, str]:
    ...
    if outcome.success:
        ...
        next_state = outcome.observed_state
        if previous_state == Monitor.State.DOWN and monitor.consecutive_successes < monitor.recovery_threshold:
            next_state = Monitor.State.DOWN
    else:
        ...
        if monitor.consecutive_failures >= monitor.failure_threshold:
            next_state = Monitor.State.DOWN
        elif previous_state in {Monitor.State.UNKNOWN, Monitor.State.UP, Monitor.State.DEGRADED, Monitor.State.MAINTENANCE}:
            next_state = previous_state if previous_state != Monitor.State.MAINTENANCE else Monitor.State.UNKNOWN
    monitor.last_checked_at = checked_at
    monitor.response_time_ms = outcome.response_time_ms
    monitor.last_message = outcome.message[:500]
    if outcome.tls_expires_at:
        monitor.tls_expires_at = outcome.tls_expires_at
    ...
    if next_state == Monitor.State.DOWN and previous_state != Monitor.State.DOWN:
        Incident.objects.create(monitor=monitor, started_at=checked_at, failure_reason=outcome.message[:500])
        transition = "DOWN"
    elif previous_state == Monitor.State.DOWN and next_state in {Monitor.State.UP, Monitor.State.DEGRADED}:
        Incident.objects.filter(monitor=monitor, ended_at__isnull=True).update(ended_at=checked_at, recovery_message=outcome.message[:500])
        transition = "RECOVERED" if next_state == Monitor.State.UP else "DEGRADED"
    elif next_state == Monitor.State.DEGRADED and previous_state != Monitor.State.DEGRADED:
        transition = "DEGRADED"
    elif previous_state == Monitor.State.DEGRADED and next_state == Monitor.State.UP:
...
async def run_monitor(monitor_id: int) -> None:
    ...
    in_maintenance = await sync_to_async(_is_in_maintenance, thread_sensitive=True)(monitor_id, checked_at)
    if in_maintenance:
        await sync_to_async(_mark_maintenance, thread_sensitive=True)(monitor_id, checked_at)
        return
    monitor = await sync_to_async(Monitor.objects.get, thread_sensitive=True)(id=monitor_id)
    if not monitor.enabled:
        return
    try:
        outcome = await perform_check(monitor)
    except Exception:
        logger.exception("Unhandled monitor check failure for monitor id %s", monitor_id)
