...
Production defaults fail closed where practical. Local development can use SQLite and an
explicit development secret, while Compose/production should provide protected environment
values.
"""
...
if DATABASE_ENGINE == "postgres":
    DATABASES = {
        "default": {
            ...
            "NAME": os.getenv("POSTGRES_DB", "goreecloud_monitor"),
            "USER": os.getenv("POSTGRES_USER", "goreecloud_monitor"),
...
MONITOR_CONTENT_SECURITY_POLICY = (
    "default-src 'self'; "
    "base-uri 'self'; "
    "connect-src 'self'; "
    "font-src 'self'; "
...
LOGGING = {
    ...
    "loggers": {
        "monitoring.wardveil": {
            "handlers": ["console"],
            "level": os.getenv("WARDVEIL_LOG_LEVEL", "INFO"),
            "propagate": False,
        }
    },
}
