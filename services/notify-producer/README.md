...
The runtime path remains disabled by default until GoreeCloud Notify is production-accepted. ntfy and Uptime Kuma were retired from the VPS on September 18, 2026 and are no longer active fallback authorities. Monitor production activation still requires cross-repository runtime validation, representative first-write/replay/conflict testing, delivery receipt, independent outage alerting, rollback/recovery evidence, the durability requirement decision, and explicit production approval.
