...
def test_repository_controls_keep_preproduction_boundary() -> None:
    ...
    assert "not yet production monitoring authority" in specs.lower()
    assert "not automatically production-accepted" in features.lower()
    assert "not production monitoring authority" in notes.lower()
