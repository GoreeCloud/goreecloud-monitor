...
`monitoring.access` records minimized JSON operational events. `MONITOR_ACCESS_LOG_LEVEL` controls its level and defaults to `INFO`. Each dynamic request receives a server-generated correlation ID exposed as `X-Request-ID`. Request events may contain event type, generated request ID, HTTP method, resolved Django route name, response status, duration, authenticated boolean, numeric user ID when authenticated, and staff flag when authenticated.

Worker and integration events record only bounded operational metadata such as worker lifecycle, numeric monitor ID, transition type/state/latency, integration name/outcome, exception class, and safe source frames. Safe source frames contain basename, line number, and function only.

The application event schema excludes raw URL paths, query strings, client IP addresses, user agents, cookies, request/response bodies, authorization headers, target URLs, monitor names in integration-failure records, usernames, credentials, exception messages, local variables, function arguments, and absolute source paths. Django's default raw-path request/server application loggers are suppressed in favor of this schema. Caddy, container-runtime, systemd/journald, and other infrastructure logging remain governed independently and must not be configured to capture reusable authorization headers.
...
The authoritative Monitor product icon is `assets/identity/goreecloud-monitor-icon.svg`, governed by the cross-platform product-identity layer. The web favicon/manifest representations, Linux/AppImage launcher input, and Android adaptive/round/monochrome launcher inputs preserve that same underlying Monitor pulse/status identity. Wardveil must not replace the Monitor application icon.

The presence of AppImage/Android launcher inputs does not claim standalone AppImage or APK/AAB client applications are implemented. Client release engineering, packaging/signing, and representative launcher acceptance remain separate delivery gates.
