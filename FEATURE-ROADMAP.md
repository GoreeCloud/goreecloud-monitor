...
| FR-005 | Add simple period + grace scheduling plus cron/time-zone scheduling; evaluate systemd OnCalendar support for practical parity. | High | Implemented in source; live acceptance pending — simple, cron/time-zone, and native systemd OnCalendar scheduling complete |
...
| FR-007 | Add run correlation/duration tracking, execution-overrun detection, Late/Started semantics, and missed-completion incidents. | High | Implemented in source; live acceptance pending — correlation, duration, overrun/missed completion, and explicit Awaiting/Started/Completed/Failed/Late presentation complete |
| FR-008 | Add scheduled-job event history, status flips, run durations, retention controls, and recovery views without becoming a general log platform. | High | Implemented in source; live acceptance pending — event history, durations, state-preserving retention, staff recovery snapshot, and versioned paginated export complete |
...
| FR-010 | Extend the versioned Manager API with scoped read-only scheduled-job summaries and bounded signal metadata under the least-privilege Manager identity. Any write administration, token rotation, or mutation API requires separate explicit Identity/Policy authorization; add read-side rate limiting when required by the accepted integration contract. | High | Partially implemented — read-only JOB list/detail and sanitized recent signal metadata complete; integration acceptance/rate-limit requirements pending; write authority intentionally not granted |
